    ...
    with torch.no_grad():
        for i, sample in enumerate(dataset):
            ...
            try:
                ...
                question = f"What is the infection risk assessment of this surgical wound?"
...
def update_excel_with_predictions(results, excel_path):
    ...
    try:
        ...
        for result in results:
            if 'predicted_answer' in result and result['predicted_answer']:
                ...
                if '. ' in predicted_text:
                    risk_level = predicted_text.split('. ')[1]
                else:
                    risk_level = predicted_text
                ...
                for sample in test_question_dataset:
                    if sample['id'] == sample_id:
                        ...
                        predictions_dict[base_name] = risk_level
                        print(f"Added prediction for {base_name}: {risk_level}")
                        break
        
        print(f"Generated predictions for {len(predictions_dict)} images")
        
        # 尝试不同的可能列名
        possible_image_cols = ['Image_Name', 'Image Name', 'image_name']
        possible_risk_cols = ['Risk_Level', 'Infection Risk Assessment', 'risk_level', 'Risk Level']
        ...
        if risk_col is None:
            print(f"Could not find risk column. Available columns: {df.columns.tolist()}")
            return
        
        print(f"Using image column: {image_col}, risk column: {risk_col}")
        
        # 更新DataFrame中的Risk_Level列
        updated_count = 0
        for idx, row in df.iterrows():
            image_name = row.get(image_col, '')
            if image_name in predictions_dict:
                df.at[idx, risk_col] = predictions_dict[image_name]
                updated_count += 1
